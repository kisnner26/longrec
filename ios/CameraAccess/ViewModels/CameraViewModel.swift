...
final class CameraViewModel {
  ...
  init(
    ...
    videoRecorder.setSegmentHandler { url in
  ...
  func stopVideoRecording() async {
    ...
    let segments = videoRecorder.segmentCount
    switch await videoRecorder.stopRecording() {
    case .completed(let url):
      if segments > 1 {
        // Grabación larga: todos los segmentos van a Fotos, sin hoja de vista previa.
        await Self.saveToPhotos(url)
        showError("Grabación guardada en Fotos en \(segments) partes.")
      } else {
  ...
  private func handleDidEnterBackground() {
    // Mientras se graba, la sesión sigue viva en segundo plano (esa es la razón de ser de la app).
    guard !isRecording else { return }
  ...
  /// Guarda un segmento en Fotos y borra el temporal. Si Fotos falla, el archivo se conserva.
  nonisolated private static func saveToPhotos(_ url: URL) async {
    guard await PHPhotoLibrary.requestAuthorization(for: .addOnly) != .denied else { return }
    do {
      ...
      // Se deja el .mov en el directorio temporal para no perder la grabación.
    }
  }
